...
BatchMandelCalculator::BatchMandelCalculator(unsigned matrixBaseSize,
    ...
    current_img_vec =
        (float *)(_mm_malloc(block_size * sizeof(float), block_size));
    line_done = (bool *)(_mm_malloc(block_size * sizeof(bool), block_size));
    int i;
...
//             }
            ...
            for (int i = 0; i < block_size; i++) {
                ...
                const float imag = y_start + iGlobal * dy;
                bool gt_4 = false;
...
#pragma omp simd reduction(& : gt_4) simdlen(128) aligned(pdata : 128)
                    for (int j = 0; j < block_size; j++) {
                        ...
                        // float current_real =
                        //     (iter == 0) ? real : current_real_vec[j];
                        // float current_imag =
                        //     (iter == 0) ? imag : current_img_vec[j];

                        float r2 = current_real_vec[j] * current_real_vec[j];
                        float i2 = current_img_vec[j] * current_img_vec[j];

                        gt_4 &= (r2 + i2 > 4.0f);

                        if (r2 + i2 > 4.0f && (pdata[iGlobal * width + jGlobal] == limit)) {
                                pdata[iGlobal * width + jGlobal] = iter;
                        }
                        else{
                            current_img_vec[j] =
                                2.0f * current_img_vec[j] * current_real_vec[j] + imag;
                            current_real_vec[j] = r2 - i2 + real;
                        }
                    }
                }
            }
